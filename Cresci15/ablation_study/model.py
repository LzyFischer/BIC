...
class InterLayer(nn.Module):
    def __init__(self, hidden_dim, num_heads, dropout, mode, inter_mode, hard_para=0.5):
        super().__init__()
        ...
        elif inter_mode == 'none':
            self.inter = InterHeadNone()
        elif inter_mode == 'hard':
            self.inter = InterHeadHard(hard_para)
        elif inter_mode == 'soft':
            self.inter = InterHeadSoft()
        elif inter_mode == 'text':
            self.inter = InterHeadText(hidden_dim=hidden_dim)
        elif inter_mode == 'graph':
            self.inter = InterHeadGraph(hidden_dim=hidden_dim)
        elif inter_mode == 'mlp':
            self.inter = InterHeadMLP(hidden_dim=hidden_dim)
...
class InterHeadNone(nn.Module):
    def __init__(self):
        super(InterHeadNone, self).__init__()
        self.head = nn.Identity()

    def forward(self, text, graph):
        return self.head(text), self.head(graph)


class InterHeadHard(nn.Module):
    def __init__(self, alpha):
        super(InterHeadHard, self).__init__()
        assert 0.0 <= alpha <= 1
        self.alpha = alpha

    def forward(self, text, graph):
        return text * self.alpha + graph * (1 - self.alpha), text * self.alpha + graph * (1 - self.alpha)


class InterHeadSoft(nn.Module):
    def __init__(self):
        super(InterHeadSoft, self).__init__()
...
class InterHeadText(nn.Module):
    ...
    def forward(self, text, graph):
        ...
        a_tt = torch.mul(text, text_sen).sum(-1)
        a_tg = torch.mul(text, graph_sen).sum(-1)
        a_gt = torch.mul(text, text_sen).sum(-1)
        a_gg = torch.mul(text, graph_sen).sum(-1)
...
class InterHeadGraph(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        self.text_fc = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.graph_fc = nn.Linear(hidden_dim, hidden_dim, bias=False)
    ...
    def forward(self, text, graph):
        ...
        a_tt = torch.mul(graph, text_sen).sum(-1)
        a_tg = torch.mul(graph, graph_sen).sum(-1)
        a_gt = torch.mul(graph, text_sen).sum(-1)
        a_gg = torch.mul(graph, graph_sen).sum(-1)
...
class InterHeadMLP(nn.Module):
    ...
    def forward(self, text, graph):
        return self.text_fc(text), self.graph_fc(graph)
...
class BIC(nn.Module):
    def __init__(self, hidden_dim, num_layers, des_dim, sta_dim, twe_dim, num_dim, cat_dim,
                 num_heads, fixed_size, graph_mode, semantic_mode, dropout, inter_mode, hard_para=0.5):
        super().__init__()
        self.layers = num_layers
        assert graph_mode in ['rgcn', 'gcn', 'gat', 'sage']
        assert semantic_mode in ['last', 'first', 'cat', 'mean', 'none']
        assert inter_mode in ['BIC', 'none', 'soft', 'hard', 'text', 'graph', 'mlp']
        ...
        for i in range(self.layers):
            self.inter.append(InterLayer(hidden_dim=hidden_dim, num_heads=num_heads, dropout=dropout, mode=graph_mode,
                                         inter_mode=inter_mode, hard_para=hard_para))
