...
all_edge_index = torch.load('../data/edge_index.pt')
all_edge_type = torch.load('../data/edge_type.pt')
...
if __name__ == '__main__':
    for p in [10, 20, 30, 40, 50, 60, 70, 80, 90]:
        graph_size = all_edge_type.shape[0] // 100 * p
        data = Data(x=uid, y=label, edge_index=all_edge_index[:, :graph_size], edge_type=all_edge_type[:graph_size])
        ...
        test_loader = NeighborLoader(data,
                                     ...
                                     shuffle=False)
        for _ in range(5):
            train('model_states_graph_{}'.format(p))
