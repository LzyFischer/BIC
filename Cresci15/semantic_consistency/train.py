...
def train(model, save_path='model_states'):
...
if __name__ == '__main__':
    for semantic_model in ['none', 'soft', 'hard', 'text', 'graph', 'mlp']:
        for fixed_size in [8, 16, 24, 32, 40]:
            for _ in range(5):

                inter_model = BIC(hidden_dim=hidden_dim, num_layers=num_layers, des_dim=768, sta_dim=768,
                                  twe_dim=768, num_dim=5, cat_dim=1,
                                  num_heads=4, fixed_size=fixed_size,
                                  graph_mode='rgcn', semantic_mode=semantic_model,
                                  dropout=dropout,)
                train(inter_model, 'inter_{}_{}'.format(semantic_model, fixed_size))
