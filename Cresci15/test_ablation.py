import torch
import numpy as np
from model import get_batch_text, BICAblation
...
def forward_one_batch(batch, model):
    ...
    return model(text, padding_mask,
                 ...
                 batch_numerical, batch_categorical, size, 'text')
...
def evaluate(model, loader):
    ...
    index = []
    for batch in loader:
        index.append(batch.x[:batch.batch_size])
...
def train():
    model = BICAblation(hidden_dim=hidden_dim, num_layers=num_layers, des_dim=768, sta_dim=768,
                        twe_dim=768, num_dim=5, cat_dim=1,
                        num_heads=4, fixed_size=fixed_size,
                        graph_mode='rgcn', semantic_mode='last',
                        dropout=dropout).to(device)
    model.load_state_dict(torch.load('model_states_text/98.13_98.54_97.40_99.70.model'))
    ...
    json.dump({
        ...
        'truth': truth,
        'preds': preds
    }, open('BIC15text.json', 'w'))

    # acc, f1, pre, rec = evaluate(model, test_loader)
    # print('training done.')
    # print('acc {:.2f} f1 {:.2f} pre {:.2f} rec {:.2f}'.format(acc, f1, pre, rec))
    # torch.save(best_model, 'model_states_graph/{:.2f}_{:.2f}_{:.2f}_{:.2f}.model'.format(acc, f1, pre, rec))
