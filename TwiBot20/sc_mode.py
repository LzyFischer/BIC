...
if __name__ == '__main__':
    for fixed_size in [16, 24, 32, 40]:
        for semantic_mode in ['cat', 'first', 'last', 'mean', 'none']:
            sc_model = BIC(hidden_dim=hidden_dim, num_layers=num_layers, des_dim=768, sta_dim=768,
                           twe_dim=768, num_dim=6, cat_dim=3,
                           num_heads=4, fixed_size=fixed_size,
                           graph_mode='rgcn', semantic_mode=semantic_mode,
                           dropout=dropout).to(device)
            train(sc_model, 'model_states_{}_{}'.format(fixed_size, semantic_mode))
