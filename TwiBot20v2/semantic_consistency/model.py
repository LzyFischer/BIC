...
class InteractModel(nn.Module):
    def __init__(self, num_property_dim=5, cat_property_dim=3, tweet_dim=768,
                 ...
                 attention_dim=16, graph_num_heads=4, dropout=0.1, consistency_mode='cat', device='cuda'):
        super(InteractModel, self).__init__()
        assert consistency_mode in ['none', 'first', 'last', 'cat', 'mean']
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, tweet_feature, des_feature, edge_index):
        ...
        elif self.consistency_mode == 'first':
            attention_vec = self.attention_linear(attention_vec_0)
        elif self.consistency_mode == 'last':
            attention_vec = self.attention_linear(attention_vec_1)
        elif self.consistency_mode == 'mean':
            attention_vec = self.attention_linear(attention_vec_0) + self.attention_linear(attention_vec_1)
        else:
            attention_vec = None

        if self.consistency_mode == 'none':
            final_input = torch.cat((title, user_feature), dim=-1)
...
class RespectiveLayer(nn.Module):
    def __init__(self, in_channels_for_graph=768, in_channels_for_text=768, out_channels=768, attention_dim=6,
                 graph_num_heads=4, text_num_heads=4, device='cuda'):
