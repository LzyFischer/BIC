...
if __name__ == '__main__':
    ...
    for _ in range(10):
        for consistency_mode, attention_dim in [('cat', 8), ('cat', 24), ('cat', 32), ('cat', 40)]:
            model = InteractModel(device=device, dropout=dropout).to(device)
            train(model, 'sc_{}_{}'.format(consistency_mode, attention_dim))
