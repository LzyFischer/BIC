...
class InteractLayer(nn.Module): 
    def __init__(self, in_channels=768, out_channels=768):
        ...
        self.linear_tmp = nn.Lineaer(in_channels*2, out_channels*2)
    ...
    def forward(self, text, all_user_feature, user_neighbor_index):
        ...
        tmp = torch.cat(text_ini, graph_ini, dim=1)
        tmp = self.linear_tmp(tmp)
        text, graph = tmp.split([self.out_channels, self.out_channels], dim=1)
