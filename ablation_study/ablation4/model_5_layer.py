...
class InteractModel(nn.Module):
    ...
    def __init__(self, num_property_dim=5, cat_property_dim=3, tweet_dim=768,
        ...
        self.Model_2 = RespectiveLayer(in_channels_for_graph=hidden_dim, in_channels_for_text=hidden_dim, out_channels=hidden_dim, attention_dim=attention_dim, graph_num_heads=graph_num_heads, device=self.device)
        self.Model_3 = RespectiveLayer(in_channels_for_graph=hidden_dim, in_channels_for_text=hidden_dim, out_channels=hidden_dim, attention_dim=attention_dim, graph_num_heads=graph_num_heads, device=self.device)
        self.Model_4 = RespectiveLayer(in_channels_for_graph=hidden_dim, in_channels_for_text=hidden_dim, out_channels=output_dim, attention_dim=attention_dim, graph_num_heads=graph_num_heads, device=self.device)
        ...
        self.InteractModel_2 = InteractLayer(in_channels=hidden_dim, out_channels=hidden_dim)
        self.InteractModel_3 = InteractLayer(in_channels=hidden_dim, out_channels=hidden_dim)

        self.attention_linear = nn.Linear(attention_dim * attention_dim * 5, output_dim // 3)
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, tweet_feature, des_feature, edge_index):
        ...
        text, all_user_feature, attention_graph_2 = self.Model_2(text, user_neighbor_index, 
                                                               all_user_feature, edge_index)
        text, all_user_feature = self.InteractModel_2(text, all_user_feature, user_neighbor_index)

        text, all_user_feature, attention_graph_3 = self.Model_3(text, user_neighbor_index, 
                                                               all_user_feature, edge_index)
        text, all_user_feature = self.InteractModel_3(text, all_user_feature, user_neighbor_index)

        text, all_user_feature, attention_graph_4 = self.Model_4(text, user_neighbor_index, 
                                                               all_user_feature, edge_index)
        ...
        attention_vec_3 = attention_graph_3.view(attention_graph_3.shape[0], self.attention_dim * self.attention_dim)
        attention_vec_4 = attention_graph_4.view(attention_graph_4.shape[0], self.attention_dim * self.attention_dim)
        
        attention_vec = torch.cat((attention_vec_0, attention_vec_1, attention_vec_2, attention_vec_3, attention_vec_4), dim=-1)
