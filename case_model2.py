...
class InteractModel(nn.Module):
    ...
    def forward(self, text, user_neighbor_index, num_feature, cat_feature, tweet_feature, des_feature, edge_index):
        ...
        text, all_user_feature, attention_graph_0, attention_weight_1 = self.Model_0(text, user_neighbor_index, 
                                                               all_user_feature, edge_index)
        text, all_user_feature,a,b,c,d = self.InteractModel_0(text, all_user_feature, user_neighbor_index)
        
        text, all_user_feature, attention_graph_1, attention_weight_2 = self.Model_1(text, user_neighbor_index, 
                                                               all_user_feature, edge_index)
...
class RespectiveLayer(nn.Module):
    ...
    def forward(self, user_text, user_neighbor_index, all_user_feature, edge_index):
        ...
        #all_user_feature = self.dropout(self.GCN(all_user_feature, edge_index))
        all_user_feature, attention_weight = self.MultiAttn(user_neighbor_index, all_user_feature)
        attention_graph = self.FixedPooling(attention)
        
        return text, all_user_feature, attention_graph, attention_weight
...
class InteractLayer(nn.Module): #大概花4min
    ...
    def forward(self, text, all_user_feature, user_neighbor_index):
        ...
        return text, all_user_feature, a, b, c, d
...
class MultiAttn(nn.Module):
    ...
    def forward(self, user_neighbor_index, all_user_feature):
        ...
        return all_user_feature, attention_weight
