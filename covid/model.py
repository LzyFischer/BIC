...
class InteractModel(nn.Module):
    ...
    def __init__(self, num_property_dim=5, cat_property_dim=3, tweet_dim=768,
                 des_dim=768, input_dim=768, hidden_dim=256, output_dim=256,
                 attention_dim=16, graph_num_heads=4, dropout=0.1, device='cuda'):
        ...
        self.Model_0 = RespectiveLayer(in_channels_for_graph=hidden_dim, in_channels_for_text=hidden_dim,
                                       ...
                                       graph_num_heads=graph_num_heads, device=self.device)
        ...
        self.attention_linear = nn.Linear(attention_dim * attention_dim * 2, output_dim)
        self.user_feature_linear = nn.Linear(output_dim, output_dim)
        self.title_linear = nn.Linear(output_dim, output_dim)  # text的长度不会在resp layer里面改变，因此还是hidden——dim

        self.final_linear = nn.Linear(output_dim * 3, 2)
...
class RespectiveLayer(nn.Module):
    ...
    def __init__(self, in_channels_for_graph=768, in_channels_for_text=768, out_channels=768, attention_dim=6,
                 graph_num_heads=4, text_num_heads=4, device='cuda:4'):
    ...
    def forward(self, user_text, user_neighbor_index, all_user_feature, edge_index):
        ...
        # for neighbor_index in user_neighbor_index:
        #     user_index.append(neighbor_index[0])
        # input_nodes = torch.tensor(user_index)
        # num_neighbors = [1]*64
        # id = torch.arange(0,500000).unsqueeze(-1).to(self.device)
        # feature_id = torch.cat([all_user_feature, id], dim=1)
        # data = Data(feature_id, edge_index)
        # loader = NeighborLoader(data, num_neighbors, input_nodes=input_nodes)
        # sampled_data = next(iter(loader))
        # feature, id = torch.split(sampled_data.x, [256, 1], dim=1)
        # index = sampled_data.edge_index
        ...
        #     all_user_feature[int(id[i])] = feature[i]
        # exit(0)
...
class InteractLayer(nn.Module):
    def __init__(self, in_channels=768, out_channels=768):
        ...
        self.text_linear = nn.Linear(in_channels, out_channels)
        self.graph_linear = nn.Linear(in_channels, out_channels)
    ...
    def forward(self, text, all_user_feature, user_neighbor_index):
        ...
        text_ini, text_rest = text.split([1, 20], dim=1)
...
class MultiAttn(nn.Module):
    ...
    def forward(self, user_neighbor_index, all_user_feature):
        for user_index in user_neighbor_index:
            tmp_feature = all_user_feature[torch.tensor(user_index)].unsqueeze(0)

            tmp_feature, attention_weight = self.multihead_attention(tmp_feature, tmp_feature,
                                                                     tmp_feature)  # batch个multi-attention
