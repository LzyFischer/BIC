...
class InterHead(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.mlp_text = nn.Linear(hidden_dim, hidden_dim)
        self.mlp_graph = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, text, graph):
        ...
        return self.mlp_text(text), self.mlp_graph(graph)
...
class BIC(nn.Module):
    def __init__(self, hidden_dim, num_layers, des_dim, sta_dim, twe_dim, num_dim, cat_dim,
        ...
        self.text_cls = nn.Linear(hidden_dim * 2, 2)
        self.graph_cls = nn.Linear(hidden_dim, 2)
    ...
    def forward(self, text, padding_mask, edge_index, edge_type, tweet, description, numerical, categorical, batch_size):
        ...
        if self.semantic_mode == 'none':
            text_final = text_code[:batch_size, 0, :]
            graph_final = graph_code[:batch_size]
        else:
            ...
            text_final = torch.cat([text_code[:batch_size, 0, :],
                                    semantic_codes[:batch_size]], dim=-1)
            graph_final = graph_code[:batch_size]

        # rep = self.act_func(self.dropout(self.fc(rep[:batch_size])))
        return self.text_cls(text_final), self.graph_cls(graph_final)


class BICAblation(nn.Module):
    def __init__(self, hidden_dim, num_layers, des_dim, sta_dim, twe_dim, num_dim, cat_dim,
                 num_heads, fixed_size, graph_mode, semantic_mode, dropout):
        super().__init__()
        self.layers = num_layers
    ...
    def forward(self, text, padding_mask, edge_index, edge_type,
                tweet, description, numerical, categorical, batch_size, ablation):
        text_code = self.text_encoding(description[:batch_size], text[:batch_size])
        graph_code = self.graph_encoding(numerical, categorical, description, tweet)
        res_text_code = text_code
        res_graph_code = graph_code
        ...
        if self.semantic_mode == 'none':
            ...
            if ablation == 'text':
                ...
                rep = torch.cat([graph_code[:batch_size],
                                 semantic_codes[:batch_size]], dim=-1)
        rep = self.act_func(self.dropout(self.fc(rep[:batch_size])))
        return self.cls(rep)
